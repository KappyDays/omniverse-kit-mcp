...
**권장 설정** (st-update.md 작성 시점 확정):

| 항목 | 값 | 이유 |
|---|---|---|
| 모델 | **Opus 4.7 (1M context)** (`claude-opus-4-7[1m]`) | 1M context 로 Plan + 여러 CLAUDE.md 동시 수용 |
| Effort | **Phase 별 조정** — 기본 `high`, Phase 2 Task 2.2-2.4 는 `max` | 판단 복잡 지점만 max (비용 효율). 상세 §4 세션 분리 가이드 |
| 권한 모드 | `acceptEdits` (`Shift+Tab` 으로 전환) | Write/Edit/Bash 매번 승인 프롬프트 제거 |

사용자가 이미 설정했으면 pass. 미설정이면 사용자에게 설정 요청 후 진행.

### Step 1 — 현재 상태 확인

아래 명령을 순서대로 실행하여 baseline 검증:

```bash
git status                                                # working tree clean 필수
git log --oneline -5                                      # HEAD + 최근 commit 확인
git branch -vv                                            # main branch, origin sync
wc -l CLAUDE.md                                           # 현재 루트 줄 수 기록
.venv/Scripts/python.exe -m pytest tests/unit/ -q        # 357 passed 유지 (불변)
.venv/Scripts/python.exe scripts/verify_mcp_sync.py       # MCP surface OK
```

- 어느 하나라도 이상하면 **작업 시작 금지, 즉시 사용자에게 보고**

### Step 2 — 현재 Phase 자동 판단 (세션 이어받기 로직)

아래 신호를 수집하여 어느 Phase 부터 시작할지 결정:

```bash
# Phase 1 완료: baseline artifacts 존재
[ -f docs/artifacts/restructure-baseline/pre/pointer_map.json ] && echo "PHASE_1_DONE" || echo "PHASE_1_TODO"
# Phase 2 완료: 루트 ≤100줄 + docs/invariants/ 존재
ROOT=$(wc -l < CLAUDE.md); [ "$ROOT" -le 100 ] && [ -d docs/invariants ] && echo "PHASE_2_DONE" || echo "PHASE_2_TODO ($ROOT lines)"
# Phase 3 완료: modules/integration-facts.md 존재
[ -f src/isaacsim_mcp/modules/integration-facts.md ] && echo "PHASE_3_DONE" || echo "PHASE_3_TODO"
# Phase 5 완료: post artifacts 존재
[ -f docs/artifacts/restructure-baseline/post/pointer_map.json ] && echo "PHASE_5_DONE" || echo "PHASE_5_TODO"
```

**판단 규칙** (위에서 아래로 — 첫 번째 적용되는 것으로 결정):

| 신호 상태 | 시작할 Phase | 첫 Task |
|---|---|---|
| `PHASE_5_DONE` | 없음 — 재구성 완료 | 사용자에게 "완료 보고, 추가 지시 요청" |
| `PHASE_1_DONE + PHASE_2_DONE + PHASE_3_DONE` (Phase 4 는 증거 미약 — git log 에서 "Phase 4 완료" 검색) | Phase 5 | Task 5.1 |
| `PHASE_1_DONE + PHASE_2_DONE + PHASE_3_DONE` | Phase 4 | Task 4.1 |
| `PHASE_1_DONE + PHASE_2_DONE` | Phase 3 | Task 3.1 |
| `PHASE_1_DONE` 만 | Phase 2 | Task 2.1 |
| 아무 신호도 없음 (clean baseline) | Phase 1 | Task 1.1 |

판단 결과를 **§9 작업 시작 선언** 의 "[Phase 판단 결과]" 블록에 기록.

### Step 3 — 필수 문서 Read (Phase 판단 결과에 맞춰)

**모든 Phase 공통**:
1. `docs/superpowers/plans/2026-04-24-claude-md-restructure-plan.md` — 메인 설계서 (1146줄, 전체)
2. `CLAUDE.md` — 현재 루트 상태
3. `isaac_extension/docs/lessons-learned.md` — L13-L17 근거 (축약 시 보존 항목)

**Phase 판단 결과별 추가 Read**:
- **Phase 2 시작 시**: `docs/artifacts/restructure-baseline/pre/*.json` (Phase 1 baseline — 하드캡 설정 / 포인터 맵 확인)
- **Phase 3 시작 시**: `docs/invariants/*.md` + `docs/runbooks/*.md` (Phase 2 산출물 — 중복 판단 기준)
- **Phase 4 시작 시**: 동일 (+ sub-CLAUDE.md 변경분 git log)
- **Phase 5 시작 시**: pre baseline + Phase 3/4 의 주요 commit diff

### Step 4 — 판단된 Phase Task 착수

§9 작업 시작 선언 보고 → 사용자 응답 대기 없이 (설정된 권한 모드 하에서) 해당 Phase 의 첫 Task 의 Step 1 부터 순차 실행.

각 Task 의 마지막 "Commit" Step 은 **사용자 승인 없이 자율 commit + push**. 단 규정된 acceptance 체크 (AC #10-#15) 를 매 commit 전 수행 — 실패 시 중단 + 사용자 보고.
...
### 3.1 현재 git 상태 (동적 참조)
- **브랜치**: `main` (origin/main 과 sync 유지 필수)
- **HEAD**: `git rev-parse HEAD` 로 확인 — baseline sha (`docs/artifacts/restructure-baseline/pre/baseline_sha.txt` 있으면 그 값) 와 같거나 이후 commit 이어야 함
- **첫 세션 시작 기준 sha** (참고용, 변경 금지): `56142f7` (2026-04-24, plan + st-update 초기 작성 완료)
- 각 세션에서 새로 추가될 commit 들은 HEAD 뒤에 쌓이고 Phase 종료 시 push 됨
...
### 세션 분리 가이드 (시나리오 B — 사용자 확정 방식)

**원칙**: Phase 경계에서 세션을 분리. Phase 내부 Task 들은 반드시 한 세션에서 연속 수행.

#### 4개 세션 구성 (확정)

| 세션 | 범위 | 예상 시간 | 권장 effort |
|---|---|---|---|
| **Session 1** | Phase 1 (test infra + baseline 캡처) | ~3h | Task 1.1-1.2: `high`, Task 1.3: `medium` |
| **Session 2** | Phase 2 (루트 재작성 + pull-doc 생성) | ~4h | Task 2.1: `high`, **Task 2.2-2.4: `max`** (판단 복잡) |
| **Session 3** | Phase 3 + Phase 4 (sub 정리 + cross-ref) | ~2.5h | `high` |
| **Session 4** | Phase 5 (post-regression) | ~1h | `high`, regression 원인 분석 시 `max` |

#### 각 세션 시작 절차 (동일)

1. 사용자: 새 Claude Code 세션 시작, `acceptEdits` 모드 확인, `@st-update.md` 주입
2. Claude: §2 Step 0 → Step 1 → Step 2 (자동 Phase 판단) → Step 3 → Step 4 순서로 실행
3. §9 "작업 시작 선언" 보고 → 해당 Phase 의 첫 Task 착수

#### 각 세션 종료 절차 (동일)

1. 현재 Phase 의 마지막 Task 의 "Commit" Step 까지 완료
2. `git status` — working tree clean 확인
3. `git push origin main` — 원격 동기화
4. 사용자에게 "§4 Phase 완료 보고 형식" 으로 메시지 → 승인 / 중단 확인
5. 세션 종료
6. **st-update.md 갱신은 불필요** — §2 Step 2 자동 판단 로직이 git/파일 상태로 다음 Phase 결정

#### 비정상 종료 시 (Phase 중간 중단)

§10.2 참조 — `git stash` + st-update.md 에 중단 기록 임시 블록 추가.
...
## 9. 작업 시작 선언 (§2 Step 0-3 완료 후, Step 4 직전에 보고)

§2 Step 0-3 완료 후 **아래 형식을 채워 사용자에게 보고**. 보고 직후 Step 4 (판단된 Phase Task 착수) 로 진행 — 사용자 추가 승인 대기 불필요 (권한 모드 `acceptEdits` 전제).

```
[세션 시작 확인]
- git HEAD: <git rev-parse HEAD 결과>
- working tree: <clean / dirty — dirty 면 원인>
- 357 unit tests: <passed / failed — failed 면 중단>
- MCP sync: <OK / FAIL — FAIL 이면 중단>
- 루트 CLAUDE.md 줄 수: <현재 wc -l>

[Phase 판단 결과 — Step 2 로직 출력]
- Phase 1: <DONE / TODO>
- Phase 2: <DONE / TODO — 현재 루트 N줄>
- Phase 3: <DONE / TODO>
- Phase 5: <DONE / TODO>
- → 시작할 Phase: <Phase N>
- → 첫 Task: <Task N.1>
...
[세션 플랜]
- 이 세션에서 수행할 Phase: <시나리오 B 기준, §4 참조>
- 예상 시간: <Session N 의 예상 시간>
- 권장 effort: <Session N 의 권장 effort>

[시작할 작업]
Phase <N> Task <N.1> — <작업 내용 요약>.
설계서 §<섹션 번호> Task <N.1> 의 Step 1-K 를 따릅니다.
```

그리고 §2 Step 4 실행 — **판단된 Phase 의 첫 Task 의 Step 1 부터 자율 실행**.

---

## 10. 이 파일의 유지보수

### 10.1 정상 진행 중 — st-update.md 갱신 불필요

§2 Step 2 의 자동 Phase 판단 로직이 **git log + 파일 시그널** 만으로 현재 Phase 를 결정하므로, 정상 진행 중에는 st-update.md 자체를 갱신할 필요 없음. 즉 매 세션 동일 파일 주입 가능.

갱신이 필요한 경우는 §10.2-§10.4 만.

### 10.2 비정상 종료 (Phase 중간 중단) 시

Phase 의 Task 중간에 중단 발생 (commit 안 된 변경 존재, 네트워크 끊김, 사용자 긴급 정지 등):

1. `git stash save "wip: Phase <N> Task <N.M> Step <S> 중단 at <YYYY-MM-DD HH:MM>"` — working tree clean 복구
2. `st-update.md` §0 바로 아래에 임시 블록 추가:
   ```markdown
   > ⚠️ **최근 중단 기록** (수동 추가 — 다음 세션이 stash pop 후 이어감):
   > - 시점: <YYYY-MM-DD HH:MM>
   > - Phase: <N> / Task <N.M> / Step <S>
   > - 마지막 완료 action: <설명>
   > - 재개 시 다음 action: <설명>
   > - `git stash list` 에 해당 wip stash 존재
   ```
3. commit + push (임시 블록 포함)
4. 다음 세션: st-update.md 읽고 §2 Step 0 앞에 이 블록 발견 → `git stash pop` 후 해당 Step 부터 재개

재개 완료 후 임시 블록은 commit 으로 제거.

### 10.3 원칙 / Plan 변경 시

사용자 지시 또는 재검토 결과:
- 절대 원칙 (§1) 변경 → §1 수정 + commit "docs(st-update): 절대 원칙 <N> 변경 — <이유>"
- Phase 구성 / Plan 변경 → plan 문서 수정이 우선. 그 후 st-update.md 에서 참조 경로 / Phase 번호만 갱신

### 10.4 재구성 완료 후 정리

Phase 5 acceptance 전원 PASS + 최종 push 완료 후:
1. `mv st-update.md archive/st-update-2026-04-23-completed.md` — 역사 기록 보존
2. `git commit -m "chore: st-update.md archived — restructure complete"`
3. `docs/artifacts/restructure-baseline/` 는 그대로 보존 (향후 유사 작업 템플릿)

### 10.5 이 파일 자체 변경 금지 조항

- Phase 판단 로직 (§2 Step 2) 의 규칙 임의 변경 금지 — plan 과 sync 필요
- "첫 세션 시작 기준 sha" (§3.1) 값 절대 변경 금지 (역사 포인터)
- "권장 설정" (§2 Step 0) 변경 시 사용자 승인 필수

---

**끝. 이 파일을 끝까지 읽었다면 §2 Step 0 부터 시작하세요.**

> **매 세션 동일 파일 주입 OK** — §2 Step 2 가 현재 Phase 를 자동 판단. 갱신은 §10.2-10.4 예외 시만.

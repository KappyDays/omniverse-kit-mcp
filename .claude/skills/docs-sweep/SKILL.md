...
# docs-sweep: Project Documentation Sweep
...
- Kit Extension catalog 갱신 → `kit-extension-catalog-sync`
...
| I2 | 다른 skill 영역 침범 금지 — `docs/references/extensions.json` / `extensions-catalog.md` (`kit-extension-catalog-sync`), `isaac_course/docs/assets/*.md` / `asset_inventory.md` (`omniverse-asset-inventory-sync`). |
...
**입력 (c) — 옵션 slash 인자**: user 가 호출 시 자유 텍스트로 (b) 를 추가 강조 가능 (예: `/docs-sweep "Phase H 완료 + stdin DEVNULL invariant 명시화"`).
...
```
🧹 docs-sweep complete — nothing to sweep
git diff HEAD: empty
```
...
```
🧹 docs-sweep complete (사후 검증 fail — stage 하지 않음)
...
- `.claude/skills/kit-extension-catalog-sync/SKILL.md` — 동형 patterns reference

...
| `RobotModule` | `robot_module.py` | load (USD payload + articulation 감지; robot payload 는 `instanceable=False`, active job 거부, playing timeline stop) / get/set_joint_positions (SingleArticulation, auto-initialize) / navigate_to (→ Job) / navigate_path (multi-waypoint → Job, timeline playing 필수) / gripper_control (finger|gripper DOF auto-detect, Franka-default 0.04/0.0 fallback) / set_ee_target (Lula IK, Franka 전용) / run_franka_pick_place (공식 PickPlaceController/RMPflow/ParallelGripper + bbox lift/place 검증) / **drive_physics (DifferentialController + Pure Pursuit, physics-based wheel joint velocities, ASYNC Job, R2 강제, 자동 wheel DOF resolve)** |

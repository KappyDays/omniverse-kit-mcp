...
- **Official asset proof gates**: Prefer `smoke/official_asset_verify_live.yaml` through `docs/invariants/scenario-validation.md` and `docs/mcp-usage-guide.md`. Success proof must assert `official_asset_verify:verification_status=load_verified`, `official_asset_verify:kind=asset`, `official_asset_verify:app_profile=isaac-sim`, and `official_asset_verify:load_quality=content_verified_no_bbox`; do not add `official_asset_verify:error_code=...` to the pass proof. Failed/timeout-shaped evidence rows may assert `error_code` only with the concrete failed `step_id` selector when the row contains one.

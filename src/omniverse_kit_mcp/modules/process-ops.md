...
- **`curl http://127.0.0.1:8111/validation/v1/health`** — 200 응답 = alive
- `netstat -ano | grep ":8011" | grep LISTENING` 없음 = endpoint 미기동
- `%TEMP%/omniverse_kit_mcp/kit_<epoch>.log` mtime 수 분째 정체

**금지** (false negative 발생): `tasklist //FI "IMAGENAME eq kit.exe"` (git bash 호출). filter 처리 timing 문제로 alive Kit 도 빈 결과 반환. 2026-04-23 사용자 검증 결과이며 현재 규칙은 `../../../docs/invariants/process-lifecycle.md` 에 보존.

...
def register_module_tools(
    ...
    *,
    ...
    async def official_asset_search(
        ...
        (
            "Search generated NVIDIA official browser-extension asset/material snapshots OFFLINE. "
            "Returns URL-based ids, provider/app evidence, stale warnings, and "
            "verify_required_before_use; verify stale/unverified hits with official_asset_verify "
            "before use. Zero-result responses include data.diagnostics.reason, "
            ...
            "data.diagnostics.status_counts, data.diagnostics.sample_names, "
            "data.diagnostics.suggested_next, and data.diagnostics.fallback_tool_order before "
            "changing filters or falling back to asset_search. Catalog-read errors use the same "
    ...
    async def official_asset_resolve(
        ...
        (
            "Resolve an official catalog name/url/id to a concrete USD or MDL target plus "
            "evidence. Prefer current app/profile loadability; if stale or not load/assign "
            "verified, verify_required_before_use is true. OFFICIAL_ASSET_NOT_FOUND and "
            "catalog-read errors include data.diagnostics.reason, "
            "data.diagnostics.candidate_counts, data.diagnostics.available_profiles, "
            "data.diagnostics.available_providers, data.diagnostics.available_kinds, "
            "data.diagnostics.status_counts, data.diagnostics.sample_names, "
            "data.diagnostics.suggested_next, and data.diagnostics.fallback_tool_order before "
            "changing kind/app_profile/prefer_loadable or falling back to asset_search."
        )
        meta = make_meta(ModuleName.ASSET)
    ...
    async def official_asset_get(
        ...
        (
            ...
            "Pass the same app_profile used for search/resolve so profile-specific latest "
            "pointers and diagnostics are used. OFFICIAL_ASSET_NOT_FOUND and catalog-read errors "
            "include data.diagnostics.reason, data.diagnostics.candidate_counts, "
            ...
            "data.diagnostics.fallback_tool_order before retrying another profile or falling "
            "back to asset_search."
        )
        meta = make_meta(ModuleName.ASSET)
    ...
    async def extension_capture_logs(
        ...
        """Peek Extension carb.log ring buffer (maxlen 10000, does not drain). Filters: ext_id substring, since_ms, level ∈ VERBOSE|INFO|WARN|ERROR|FATAL|ALL. Use extension_clear_logs before risky live work to start a request-scoped capture window; set stop_after_capture=True after collecting failure logs. For stop_after_capture=True, inspect data.capture_stop_requested, data.capture_stop_completed, data.capture_stop_timed_out, and data.capture_running before assuming the hook closed."""
        meta = make_meta(ModuleName.EXTENSION)
    ...
    async def sensor_lidar_get_point_cloud(
        ...
        """Read one frame of RTX Lidar XYZ point cloud (symmetric readback for sensor_attach_rtx_lidar). Reuses annotator stamped on sensor prim. Empty/short/warned reads and hard read errors return diagnostics.suggested_next and diagnostics.fallback_tool_order; scenario reports promote these to diagnostic_next_actions. Set min_points>0 or fail_on_warning=True to fail live proof loops instead of silently accepting empty/warned data. Short reads expose diagnostics.reason=point_count_below_minimum plus diagnostics.num_points/min_points; warning failures use SENSOR_LIDAR_POINT_CLOUD_WARNING with diagnostics.reason=lidar_warning; hard read errors use diagnostics.reason=lidar_read_error. Truncates to max_points (<=100000)."""
        meta = make_meta(ModuleName.SENSOR)

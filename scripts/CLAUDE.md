...
| `probe_mcp_surface.py [--workspace workspaces/isaac/instance-1] [--runtime-info --expect-tool-profile full --expect-app-profile isaac-sim --expect-tool-count 152 --require-runtime-fresh] [--scenario-plan smoke/foo.yaml --require-plan-fields --require-live-validation-tools a,b,c]` | Stdio MCP smoke for tools/resources plus optional `mcp_runtime_info` and `scenario_plan` result-shape/checklist-order checks through the workspace-local entry | Use from parent/root to verify MCP surface, profile/import freshness, plan fields, and live wrapper order without live stage mutation when first-class live tools are not exposed in the current host |

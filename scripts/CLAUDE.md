...
| `diff_catalog.py` | 현재 `extensions.json` vs fresh harvest 비교 (added / removed / version_bumped / category_changed) | Kit / app 버전 bump 후 sync 필요 여부 판정 — workflow 는 `docs/references/CLAUDE.md` "Kit / app 업데이트" 섹션 |
| `diff_asset_inventory.py` | `isaac_course/docs/assets/*.md` 의 모든 USD URL 을 NVIDIA S3 에 HTTP HEAD 검증. 404 / NET / 5xx 보고 | Asset 경로 fail 보고 시 또는 Isaac Sim 5.x 패치 후 — workflow 는 `/asset-inventory-sync` skill |
...
- Scenario runner 내부: `../src/isaacsim_mcp/scenario/CLAUDE.md`

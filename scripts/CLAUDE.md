...
| `run_scenario_standalone.py [--dry-run] [--input-overrides-json {...}] <scenario_path>` | Compile or run scenario runner with the latest `src/` code | Bypass MCP import cache; use `--dry-run` for plan/evidence/retry preflight without stage mutation, or run normally for scenario live verification |

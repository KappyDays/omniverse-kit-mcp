...
| `probe_mcp_surface.py [--workspace workspaces/isaac/instance-1] [--scenario-plan smoke/foo.yaml --require-plan-fields] [--require-plan-field evidence_steps]` | Stdio MCP smoke for tools/resources and optional `scenario_plan` result-shape checks through the workspace-local entry | Use from parent/root to verify MCP surface and plan fields without live stage mutation when first-class live tools are not exposed in the current host |

...
| `review_public_hygiene.py` | Scan current tracked plus untracked non-ignored files and pending/session commit history for public path/secret/generated-reference leaks | Run before public commit/push; pass `--base <base> --head HEAD` for explicit ranges, `--today --head HEAD` for the current day, or `--since "YYYY-MM-DD 00:00"` for older session/day audits |

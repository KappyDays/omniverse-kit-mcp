...
| `probe_mcp_surface.py [--workspace workspaces/isaac/instance-1] [--runtime-info --expect-tool-profile full --expect-app-profile isaac-sim --expect-tool-count 152 --require-runtime-fresh] [--scenario-plan smoke/foo.yaml --require-plan-fields --require-live-validation-tools a,b,c --expect-scratch-stage-required true --expect-log-capture-recommended true --expect-retry-key-arg step:key=value]` | Stdio MCP smoke for tools/resources plus optional `mcp_runtime_info` and `scenario_plan` result-shape/checklist-order/scratch-read-only/retry-key-arg checks through the workspace-local entry | Use from parent/root to verify MCP surface, profile/import freshness, plan fields, live wrapper order, and retry threshold overrides without live stage mutation when first-class live tools are not exposed in the current host |

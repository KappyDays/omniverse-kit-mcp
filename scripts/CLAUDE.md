...
- **MCP import cache 우회**: `src/isaacsim_mcp/` 코드를 수정하면 Claude Code 재시작 전까지 MCP tool 호출로는 반영되지 않는다. `run_scenario_standalone.py` / `run_process_module_standalone.py` 는 매 실행마다 fresh Python process 로 import 하므로 최신 코드가 즉시 반영됨. Extension 코드 변경 (`kkr-extensions/`) 은 `isaac_sim_restart` 로 즉시 반영.

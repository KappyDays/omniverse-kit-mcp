...
- **LakehouseModule** query only (인터뷰 스펙 확정) — 본문 `src/isaacsim_mcp/modules/CLAUDE.md` LakehouseModule
- **Type boundary**: 내부 `@dataclass(slots=True, frozen=True)`, Pydantic 은 Extension REST 경계만. MCP 서버 코드 Pydantic 금지 — 본문 `src/isaacsim_mcp/CLAUDE.md` "Type Boundary Convention"

...
| 작업 | 먼저 Read |
|---|---|
| 씬 빌드 / asset 선택 (robot·character·env·prop 추가) | `docs/invariants/asset-discovery.md` |
...
| 세션 작업 후 문서 hierarchy sweep | skill `/omniverse-docs-sweep` (`.claude/skills/omniverse-docs-sweep/SKILL.md`) |
...
장애 진단은 `docs/runbooks/` (kit-stdin-deadlock · cold-boot-timeout · hub-orphan · env-sub-config · kit-dep-solver-fail · multi-app · scene-reexport-lock).
...
| `ISAAC_SIM_EXTRA_EXT_FOLDERS` | `[]` | JSON array. out-of-tree extension 폴더마다 `--ext-folder` 추가 (외부 extension 폴더 영구 등록). 빈 list 면 현행 동일 |
...
| `docs/assets/isaac/` | NVIDIA Isaac Sim 6.0 asset URL 카탈로그 SoT (`asset_inventory.md` 진입점 + per-category `assets/*.md`) — 진입 워크플로 `docs/invariants/asset-discovery.md` |

## 메타 — CLAUDE.md 작성 규칙

- **라인 하드캡**: 루트 ≤150 · sub-CLAUDE.md ≤150 · `docs/invariants/*.md` ≤200 · `docs/runbooks/*.md` ≤300 (`test_doc_integrity.py` A3/A4/A6/A8/A9 가드, env override 가능)

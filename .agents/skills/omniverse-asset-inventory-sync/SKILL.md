...
**Goal**: Verify that all USD URLs in both inventory directories are still valid in NVIDIA S3 and fix invalid entries. Prevent stale paths from accumulating after Isaac Sim 5.x patch/SimReady release.

**Watched scopes** (both same `omniverse-content-production` S3 bucket):
- `docs/assets/isaac/assets/*.md` — Isaac Sim 5.1 bundle only (strict: only accepts `Isaac/5.1` or `simready_content` prefix)
...
| I4 | Pre-commit: `pytest tests/unit/test_asset_inventory_integrity.py -q` green, `verify_mcp_sync.py` OK, root `AGENTS.md` ≤ 100 lines. |
...
> "It appears that NVIDIA has moved to a new Isaac Sim asset bucket (e.g. `5.1` → `5.2`). Shall we update the `$ISAAC` prefix of all sub-mds to `Isaac/5.2/Isaac` at once?"
...
Integrity: pytest 7 passed, verify_mcp_sync OK, AGENTS.md <N> lines
```

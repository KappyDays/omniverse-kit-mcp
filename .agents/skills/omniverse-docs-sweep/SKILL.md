...
**목표**: 직전 작업의 `git diff HEAD` + conversation 결정 사항을 분석해 root AGENTS.md "변경 파급 매트릭스" + sub-AGENTS.md / `docs/invariants/` / `docs/runbooks/` / 카운터·pointer / 표 항목을 자동 동기화. 영구 규칙·신규 문서 후보는 dry-run 으로 보고만 한다 (silent 자동 추가 금지). 매트릭스를 SoT 로 두고 skill 안에 룰을 hard-code 하지 않는다.

## When to Use

User says "docs sweep", "문서 갱신", "docs 동기화", "AGENTS.md 정리" 등. **Skip** for:
...
| I2 | 다른 skill 영역 침범 금지 — `docs/references/extensions.json` / `extensions-catalog.md` (`omniverse-kit-extension-catalog-sync`), `docs/assets/isaac/assets/*.md` / `asset_inventory.md` (`omniverse-asset-inventory-sync`). |
| I3 | L3 (영구 규칙·신규 디렉토리·신규 sub-AGENTS.md·`lessons-learned.md` 영구 규칙·매트릭스 자체 수정·stale 아닌 삭제) 는 dry-run only — user 승인 없이 편집 금지. Sign-off 의 후보 리스트로만 보고. |
...
| I5 | 매트릭스가 SoT — 변경 파급 룰을 skill 안에 hard-code 금지. root `AGENTS.md` "변경 파급 매트릭스" 표를 매번 parse. parse fail 시 STOP (skill stale 신호). |
...
root `AGENTS.md` 의 "변경 파급 매트릭스" 표를 Read + parse. 각 행은 `| 변경 대상 | 함께 수정 |` 형식. Step 1 의 변경 파일을 매트릭스 행과 매칭해 *함께 갱신해야 할 후보 docs* 추출.
...
- **(3a) 카운터**:
  - tool 수: `grep -c '@mcp.tool()' src/omniverse_kit_mcp/tools/*.py` 합산 → root `AGENTS.md` / `docs/tools-roadmap.md` Implementation Status 섹션의 수치 확인
  - test 수: `pytest --collect-only -q` 마지막 줄
  - 라인 수: `wc -l` 로 각 AGENTS.md / invariants / runbooks
- **(3b) Pointer**: 모든 sub-AGENTS.md / invariants / runbooks 의 cross-ref (`docs/...`, `../...`) 추출 → 파일 존재 검증. 깨진 pointer 중 *단순 rename / path drift* 만 후보 (의미 변형 동반은 L3).
- **(3c) 표 항목 누락**: sub-AGENTS.md "파일 구조" 표 / root AGENTS.md "문서 맵" 표 / pull-doc 표 에 Step 1 의 신규 파일이 누락됐는지 검증.
...
- 신규 sub-AGENTS.md untracked
- `lessons-learned.md` 에 영구 규칙 추가 시도 검출 (메타 룰 위반 — incident log only)
- root AGENTS.md 메타 룰 `삭제는 stale 한정` 위반 가능성 검출
- root AGENTS.md "변경 파급 매트릭스" 자체 수정 후보
...
- **라인캡**: 루트 `AGENTS.md` ≤100 / sub-AGENTS.md ≤150 / `docs/invariants/*.md` ≤200 / `docs/runbooks/*.md` ≤300 — 하나라도 초과 시 후보 폐기
...
- **메타 룰 위반**:
  - `lessons-learned.md` 에 영구 규칙 추가? (root AGENTS.md 메타 — incident log only)
  - sub-AGENTS.md 에 cross-cutting 룰 추가? (sub 는 디렉토리 고유 룰만)
  - stale 한정이 아닌 삭제?
...
- Step 2: 매트릭스 parse fail → STOP (root AGENTS.md schema 변형 의심, skill 코드 수정 필요 가능성)
...
- ❌ 다른 skill 영역 (`docs/references/extensions.json` / `extensions-catalog.md`, `docs/assets/isaac/assets/*.md` / `asset_inventory.md`) 편집
...
- ❌ root `AGENTS.md` "변경 파급 매트릭스" 자체 자동 수정 (매트릭스 자체 변경은 L3)
...
- root `AGENTS.md` — "변경 파급 매트릭스" (SoT) + 메타 룰 (라인 하드캡, 이관/삭제 룰, lessons-learned incident log) + DO-NOT-EDIT 보호 G1-G7
- `docs/AGENTS.md` — docs 디렉토리 각 파일의 역할 + 업데이트 규칙
- `tests/unit/test_doc_integrity.py` — 라인캡 / cross-ref / G1-G7 가드
- `scripts/verify_mcp_sync.py` — MCP catalog drift 가드
- `.Codex/skills/omniverse-kit-extension-catalog-sync/SKILL.md` — 동형 patterns reference
- `.Codex/skills/omniverse-asset-inventory-sync/SKILL.md` — 동형 patterns reference

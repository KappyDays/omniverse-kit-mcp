...
**Goal**: **Detailed analysis of the omniverse tasks (implementation content, called MCP tool, bypass/manual steps, repetitive friction) performed in this Claudee Code / Codex session**, identify gaps in the MCP surface, and — if **new tool addition or existing tool upgrade** is required — autonomously complete 7 implementations + registration/drift verification + tool-specific docs sync. **The input is the actual task of the session (conversation + git diff), not a task specification given externally**. The SoT of the procedure is an existing invariant document, and this skill is only *pointed to* (no hard-code).
...
- Extensive AGENTS.md layer synchronization → `omniverse-docs-sweep`
...
## Required Read upon entry (autonomous mode — do not autoload sub-AGENTS.md)

- `docs/references/AGENTS.md` — research flow steps 0 to 6 (duplicate check → catalog → hint → API → example → source → document)
...
4. Execute research flow (`docs/references/AGENTS.md` step 1~6) for each gap → Confirm ext/API to be wrapped. When in-process path is discovered through research, provisional (e) → (a)/(b) reclassification; (e) Confirmed only when route absence is confirmed.
...
- **P5 docs (tool only)**: tool-catalog regen is included in P4-(i) `verify_mcp_sync` (group caveat·frozenset has already been completed in 7 places in P3). P5 unique task = Check for additional updates in the change propagation matrix. Extensive AGENTS.md layer synchronization causes `omniverse-docs-sweep` handoff.
...
- ❌ Extensive AGENTS.md synchronization (→ docs-sweep), catalog/asset area invasion
...
- `.agents/skills/omniverse-docs-sweep/SKILL.md`, `.agents/skills/omniverse-kit-extension-catalog-sync/SKILL.md` — isomorphic skill structure patterns reference

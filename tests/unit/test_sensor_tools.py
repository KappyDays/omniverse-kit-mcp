...
@pytest.mark.asyncio
async def test_attach_rtx_lidar_error_returns_typed_diagnostics():
    class BrokenClient:
        async def sensor_attach_rtx_lidar(self, _req):
            raise RuntimeError("RTX lidar extension unavailable")

    module = SensorModule(BrokenClient())  # type: ignore[arg-type]
    request = SensorAttachRtxLidarRequest(
        robot_prim="/World/Robot",
        mount_offset=(0.0, 0.0, 1.0),
        mount_rotation=(0.0, 0.0, 0.0),
        config_preset="Example_Rotary",
        sensor_name="TopLidar",
    )
    ...
    assert not result.ok
    assert result.status == ExecutionStatus.ERROR
    assert result.error_code == "SENSOR_ATTACH_RTX_LIDAR_ERROR"
    assert "RTX lidar extension unavailable" in (result.message or "")
    assert isinstance(result.data, SensorAttachRtxLidarResult)
    assert result.data.ok is False
    assert result.data.parent_prim == "/World/Robot"
    assert result.data.config_preset == "Example_Rotary"
    assert result.data.diagnostics["reason"] == "rtx_lidar_attach_error"
    assert result.data.diagnostics["upstream_error_code"] == (
        "SENSOR_ATTACH_RTX_LIDAR_ERROR"
    )
    assert result.data.diagnostics["fallback_tool_order"] == [
        "mcp_runtime_info",
    ...
    assert any(
        "stage_capture_snapshot" in item
        for item in result.data.diagnostics["suggested_next"]
    )

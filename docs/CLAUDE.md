...
| `invariants/` | **작업 전 필독 pull-doc** — USD 로드 / process lifecycle / MCP tool 추가 / module 추가 / extension reload / UI invoke / scenario validation / multi-app / visual-validation 9 개. 루트 CLAUDE.md 의 "⚠️ 작업 전 필수 pull-doc" 표가 진입점 | 하드캡 ≤200 줄. 영구 규칙 신규 추가 시 여기 신설 |
...
| `artifacts/` | live validation 산출물 (`constraint-validation-2026-04-23/`). validation 스크립트가 쓰는 결과 저장소 | live test 스크립트가 직접 쓰기 |

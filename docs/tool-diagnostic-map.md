...
| Robot direct control failed? | `mcp_runtime_info` → `simulation_get_status` → `stage_capture_snapshot` → `robot_get_joint_config_static` if IK-related → retry `robot_gripper_control` / `robot_set_ee_target` → `extension_capture_logs` | Inspect `diagnostics.reason=robot_gripper_control_error` / `robot_set_ee_target_error`, `diagnostics.prim_path`, action/target or target_pose fields, `diagnostics.upstream_error_code`, and `diagnostics.fallback_tool_order` |

...
| Robot navigation or EE pose failed? | `mcp_runtime_info` → `simulation_get_status` → `stage_capture_snapshot` → `robot_get_joint_config_static` if pose-related → retry `robot_navigate_to` / `robot_navigate_path` / `robot_get_ee_pose` → `job_status` if a job was created → `extension_capture_logs` | Inspect `diagnostics.reason=robot_navigate_to_error` / `robot_navigate_path_error` / `robot_get_ee_pose_error`, requested target/waypoint/frame fields, `diagnostics.upstream_error_code`, and `diagnostics.fallback_tool_order` |

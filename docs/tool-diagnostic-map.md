...
| Log capture failed? | `extension_clear_logs` → retry `extension_capture_logs` → `process_list_kit_instances` → `kit_app_restart` only after a confirmed log-endpoint hang | For `EXTENSION_LOGS_ERROR`, inspect `data.diagnostics.reason=extension_logs_error`, `data.diagnostics.error_type`, `data.diagnostics.retryable`, and `data.diagnostics.fallback_tool_order`; for `ok=true` captures with `stop_after_capture=true`, inspect `data.capture_stop_timed_out` and `data.capture_running` before assuming the hook closed |
| Robot profile probe failed before load? | `robot_list_arm_profiles` → retry `robot_probe_arm_profile` with a listed profile → `robot_probe_arm_profiles` for bounded comparison → `official_asset_search` / `asset_search` → `robot_load` for direct USD assets | For `ROBOT_PROBE_UNKNOWN_PROFILE`, inspect typed error `data.checks.probe.evidence.requested_profile_found=false`, `data.checks.probe.evidence.known_profile_count`, `data.checks.probe.evidence.suggested_next`, and `data.checks.probe.evidence.fallback_tool_order` before changing assets or retrying broadly |
...
| Pick-place playback profile load/install/reset failed? | `robot_list_arm_profiles` → `mcp_runtime_info` → `simulation_get_status` → `stage_capture_snapshot` → `robot_load` if profile asset load failed → retry `robot_install_pick_place_playback_demo` / `robot_reset_pick_place_demo` → `robot_get_pick_place_demo_status` → `extension_capture_logs` | Inspect `diagnostics.reason=pick_place_profile_load_error` / `pick_place_profile_asset_not_articulation` / `pick_place_demo_install_error` / `pick_place_demo_reset_error`, selected profile/asset URL, requested robot/object/target/fit fields, `diagnostics.upstream_error_code`, and `diagnostics.fallback_tool_order` |
...
| RTX lidar point-cloud proof failed? | `scenario_last_report(report_format="markdown", redact_local_paths=true)` for triage → default JSON `scenario_last_report(redact_local_paths=true)` for exact public-safe fields → `simulation_get_status` → retry `sensor_lidar_get_point_cloud` only after checking plan retry args → `extension_capture_logs` | For empty/short reads inspect `diagnostics.reason=point_count_below_minimum`; for `fail_on_warning=true` warning failures inspect `SENSOR_LIDAR_POINT_CLOUD_WARNING` and `diagnostics.reason=lidar_warning`; for hard read errors inspect `diagnostics.reason=lidar_read_error`; always check `diagnostics.num_points`, `diagnostics.min_points`, `empty_reason`, `retry_failures[].data_summary`, `diagnostic_next_actions`, and `diagnostics.fallback_tool_order`; controlled-failure probe wrappers should pin this with `--expect-live-diagnostic-field read_lidar_point_cloud:diagnostics.reason=point_count_below_minimum`, `--expect-live-diagnostic-field read_lidar_point_cloud:diagnostics.min_points=513`, and `--expect-live-diagnostic-field read_lidar_point_cloud:diagnostics.fallback_tool_order='["simulation_step","sensor_lidar_get_point_cloud","extension_capture_logs"]'` |
...
| Official asset verify failed or found nothing? | `official_asset_sync_status` → `official_asset_search` → `official_asset_resolve` → `official_asset_get` → retry `official_asset_verify` only after bounded diagnostics → `scenario_last_report(report_format="markdown", redact_local_paths=true)` for public triage and default JSON `scenario_last_report(redact_local_paths=true)` for exact public-safe fields if running the same scenario wrapper host | For `OFFICIAL_ASSET_NOT_FOUND`, inspect `diagnostics.reason`, `diagnostics.candidate_counts`, `diagnostics.available_profiles`, `diagnostics.available_providers`, `diagnostics.suggested_next`, and `diagnostics.fallback_tool_order`; `evidence_summary` should stay empty because no stage probe ran. For failed verify records inspect `diagnostics.asset_checks` / `diagnostics.material_checks`, `diagnostics.error_type`, `evidence_summary[].evidence_kind=official_asset_verify`, and `evidence_summary[].error_code` when present before retrying or falling back. The read-only diagnostic probe should pin `--expect-live-failure-step-error get_pallet_wrong_profile=OFFICIAL_ASSET_NOT_FOUND`, `--expect-live-diagnostic-field search_known_miss:diagnostics.reason=query_no_match`, `--expect-live-diagnostic-field get_pallet_wrong_profile:diagnostics.reason=app_profile_not_covered`, `--expect-live-diagnostic-field search_known_miss:diagnostics.fallback_tool_order='["official_asset_sync_status","official_asset_search","official_asset_resolve","official_asset_verify","asset_search"]'`, and `--expect-live-diagnostic-field get_pallet_wrong_profile:diagnostics.fallback_tool_order='["official_asset_sync_status","official_asset_search","official_asset_resolve","official_asset_verify","asset_search"]'` |
...
| Viewport comparison failed? | `viewport_capture_assert` for both images → retry `viewport_compare_ssim` → `extension_capture_logs` | For `VIEWPORT_COMPARISON_ERROR`, inspect `diagnostics.reason=viewport_compare_ssim_error`, artifact path presence booleans, `diagnostics.min_ssim`, `diagnostics.crop`, `diagnostics.upstream_error_code`, and `diagnostics.fallback_tool_order` before regenerating evidence |
...
| Viewport window/render setting failed? | `mcp_runtime_info` → `simulation_get_status` → `window_list` for create/destroy or retry the failing `viewport_set_render_*` / `viewport_toggle_overlay` / `viewport_set_fov` tool → `viewport_capture_assert` when render changed → `extension_capture_logs` | Inspect `diagnostics.reason=viewport_create_error` / `viewport_destroy_error` / `viewport_set_render_mode_error` / `viewport_set_render_quality_error` / `viewport_toggle_overlay_error` / `viewport_set_fov_error`, `diagnostics.request`, `diagnostics.upstream_error_code`, and `diagnostics.fallback_tool_order` |

...
After the final log capture, record `data.capture_stop_timed_out=false` or `data.capture_running=false` before treating the request-scoped log hook as closed.

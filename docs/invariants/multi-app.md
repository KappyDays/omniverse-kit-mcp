...
Guard function 은 `isaac_extension/.../validation_api/_app_features.py` 의
...
dependencies" 로 crash. Config validator (`config.py::_resolve_profile_and_derived_fields`)
가 profile=isaac-sim 에만 env 값 적용, 그 외는 profile 의 curated
extra_ext_ids 사용.
...
2. `supported_module_groups` 에 해당 app 이 실제 지원하는 group 만 포함.
   지원 안 하는 group 은 extension 측 `_app_features.py` 의 guard 가 자동
   503 처리 → 추가 코드 변경 불필요
...
- Extension guards: `isaac_extension/omni.mycompany.validation_api/.../_app_features.py`

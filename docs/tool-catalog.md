...
channel so you can auto-detect a blank/black frame without reading the PNG. Failures include
...
failure logs. For stop_after_capture=True, inspect data.capture_stop_requested,
data.capture_stop_completed, data.capture_stop_timed_out, and data.capture_running before
assuming the hook closed.
...
below pick/place validation. Unknown profiles return error_code=ROBOT_PROBE_UNKNOWN_PROFILE
with typed data.checks.probe.evidence.fallback_tool_order. timeout_s defaults to 90 seconds to
record slow profiles instead of hanging the MCP caller; pass null only for deliberate unbounded
diagnostics. Set dynamic_checks=false for load/articulation/static-metadata hazard triage. Set

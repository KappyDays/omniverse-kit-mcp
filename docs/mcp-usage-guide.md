...
`stage_mutation_steps`, `evidence_steps`, and `retry_steps` so a missing setup,
stage/sensor mutation, viewport cleanup, lidar evidence, capture evidence, or
idempotent retry gate is caught before live stage mutation. `stage_mutation_steps`
must match the scratch/test stage you intend to touch. For retried evidence steps, inspect
...
`stage_mutation_steps` for stage/live-scene side effects, `evidence_steps` for
...
Before live execution, confirm `scenario_plan.stage_mutation_steps` includes the
...
`stage_mutation_steps` should be empty.

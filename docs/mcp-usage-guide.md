...
| Prove the robot + RTX sensor golden path | `scenario_plan(smoke/robot_rtx_sensor_golden_workflow.yaml)`, `scenario_validate(smoke/robot_rtx_sensor_golden_workflow.yaml)`, `scenario_last_report` | `docs/invariants/scenario-validation.md`, `src/omniverse_kit_mcp/modules/integration-facts.md` |
...
| Attach RTX sensors to a robot manually | Prefer the smoke scenario route; manual first tools are `robot_load`, `stage_create_prim`, `simulation_play/stop`, `sensor_attach_rtx_camera`, `sensor_set_annotator`, `sensor_attach_rtx_lidar`, `sensor_lidar_get_point_cloud`, `viewport_capture_assert` | `docs/invariants/scenario-validation.md`, `src/omniverse_kit_mcp/modules/integration-facts.md` |

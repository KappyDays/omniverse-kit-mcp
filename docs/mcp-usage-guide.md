...
| Work with robot or character motion | `robot_list_arm_profiles`, `robot_load`, `robot_probe_arm_profile`, `robot_install_pick_place_playback_demo`, `robot_get_pick_place_demo_status`, `character_load`, `job_status`; choose `support_status=validated_pick_place` before pick/place playback, and inspect unsupported/candidate/status-timeout `diagnostic_next_actions`, `diagnostics.target_status`, `diagnostics.timeout_s`, and `diagnostics.fallback_tool_order` before claiming proof or retrying broadly | `src/omniverse_kit_mcp/modules/CLAUDE.md`, `docs/invariants/scenario-validation.md` |

...
| Work with robot or character motion | `robot_list_arm_profiles`, `robot_load`, `robot_probe_arm_profile`, `robot_get_pick_place_demo_status`, `character_load`, `job_status`; for pick/place status timeouts, inspect `diagnostic_next_actions`, `diagnostics.timeout_s`, and `diagnostics.fallback_tool_order` before claiming proof or retrying broadly | `src/omniverse_kit_mcp/modules/CLAUDE.md`, `docs/invariants/scenario-validation.md` |

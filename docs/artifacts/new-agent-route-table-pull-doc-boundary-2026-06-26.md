...
from `docs/mcp-usage-guide.md` and find the scenario validation invariant plus
scenario authoring guide before attempting Robot + RTX or official asset live
proofs.
...
- `tests/unit/test_doc_references.py` guards both route rows so the entry-point
  table cannot silently drop the durable scenario proof docs.

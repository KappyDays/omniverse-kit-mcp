...
(예: `"<old-repo>/kkr-extensions"`) 가 디렉토리
...
- 옛 경로 `<old-repo>/kkr-extensions/` 가 (사고 시점) stale,
  실 extension 들은 `<repo>/kkr-extensions/`
  에 위치 → solver 가 아무 것도 못 찾아 종료

> ⚠️ 정정 (2026-05-26): 위는 2026-05-04 사고 시점 기록. 현재 `<old-repo>/`
...
- rename 직후 `grep -rn '"<workspace>/' --include='*.kit' branch/`

<!-- SoT: src/isaacsim_mcp/modules/process_module.py::ProcessModule.start + types/profile.py + .env -->
...
| 항목 | 값 | 비고 |
|------|-----|------|
| `--ext-folder` | `C:/Users/<you>/workspace/Isaac-sim-MCP/isaac_extension` | 모든 profile 동일 |
...
| `stdout` / `stderr` | `%TEMP%/isaacsim_mcp/kit_<epoch>.log` | OS pipe 버퍼 포화 → kit hang 방지 |
...
& "C:/Users/<you>/workspace/branch/isaac-sim-standalone-5.1.0-windows-x86_64/kit/kit.exe" `
  ...
  --ext-folder "C:/Users/<you>/workspace/Isaac-sim-MCP/isaac_extension" `
  ...
  *> "$env:TEMP/isaacsim_mcp/kit_isaac_$(Get-Date -UFormat %s).log" `
  < $null
```
...
& "C:/Users/<you>/workspace/branch/kit-app-template/_build/windows-x86_64/release/kit/kit.exe" `
  ...
  *> "$env:TEMP/isaacsim_mcp/kit_usdcomposer_$(Get-Date -UFormat %s).log" `
  < $null
```
...
- ProcessModule SoT: `src/isaacsim_mcp/modules/process_module.py`
- Profile SoT: `src/isaacsim_mcp/types/profile.py`
...
- `[settings.app.exts.folders] '++'` 배열에 `"C:/Users/<you>/workspace/Isaac-sim-MCP/isaac_extension"` 추가
...
- **ext-folder 가 절대경로** (`C:/Users/<you>/workspace/Isaac-sim-MCP/isaac_extension`) → 프로젝트를 옮기면 `.kit` 도 함께 갱신

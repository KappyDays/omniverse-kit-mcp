...
4. **`~/.claude.json` 등록** — `mcpServers.omniverse-kit-mcp` entry 추가/갱신

## 🚨 MCP 서버 등록 위치 (주의)

- **등록 파일**: `~/.claude.json` (= `$env:USERPROFILE\.claude.json`)
- **NOT** `~/.claude/settings.json` — Claude Code 는 오직 `.claude.json` 한 파일에서 MCP 서버를 읽는다
- 기존 `~/.claude.json` 이 있으면 `mcpServers` 키에 `omniverse-kit-mcp` entry 를 병합 (덮어쓰기 아님)
...
2. `omniverse-kit-mcp` tool prefix 로 도구들이 보이는지 확인

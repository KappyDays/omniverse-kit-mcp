...
$kitFile = Join-Path $scriptDir "apps/kkr_usd_composer.kit"
$portByInstance = @{
    1 = 8114
    2 = 8115
}
...
for ($i = 0; $i -lt $args.Count; $i++) {
    ...
    switch ($arg) {
        ...
        "--port" {
            if ($i + 1 -ge $args.Count) {
                throw "--port requires 8114 or 8115"
            }
            $i++
        ...
        "--help" {
            Write-Host "Usage: kkr_usd_composer_mcp.kit.bat [--instance 1|2] [--port 8114|8115] [--dry-run] [USD Composer args...]"
            exit 0
        }
        default {
...
if ($requestedPort -ne $null -and -not ($portByInstance.Values -contains $requestedPort)) {
    throw "--port must be 8114 or 8115"
}
...
function New-PortMutex {
    ...
    $created = $false
    $mutex = [System.Threading.Mutex]::new($true, "Global\UsdComposerMcpPort$Port", [ref]$created)
    if (-not $created) {
        $mutex.Dispose()
        return $null
    }
...
function Select-Port {
    ...
    throw "No USD Composer MCP port available. Tried: $($candidates -join ', ')"
}
...
try {
    ...
    Write-Host "Starting USD Composer MCP instance $($selection.Instance) on port $($selection.Port)"
    & $kitExe @kitArgs
    exit $LASTEXITCODE
} finally {

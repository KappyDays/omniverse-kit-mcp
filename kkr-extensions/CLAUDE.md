...
| `omni.mycompany.robot_lidar/` | Hospital + Nova Carter + Turtlebot3 with shared top-down RTX LiDAR viewport. NavMesh random-goal drive (DifferentialController), Velodyne-class 128ch LiDAR with ray + hit-point debug visualization. **독립 구조** — copies navmesh_playground patterns. self-test stamps `/RobotLidar/SelfTestResult`. |

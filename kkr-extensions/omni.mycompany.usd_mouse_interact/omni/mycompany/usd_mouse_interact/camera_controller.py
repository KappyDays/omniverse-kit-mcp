...
class CameraController:
    ...
    def activate(self) -> bool:
        ...
        if active_viewport is None:
            carb.log_warn("usd_mouse_interact: no active viewport on activate")
            return False
        camera_path = active_viewport.get_active_camera()
        if not camera_path:
            carb.log_warn("usd_mouse_interact: active viewport has no active camera")
            return False
        ...
        if flen > 1e-9:
            ...
            carb.log_warn(
                "usd_mouse_interact: activate — zero-length forward vector "
    ...
    def deactivate(self) -> None:
        ...
        try:
            ...
            carb.log_warn(f"usd_mouse_interact: camera restore failed: {exc}")
        self._orig = None
    ...
    def apply(self, dx_pixels: float, dy_pixels: float, keys: camera_math.MovementInput,
        ...
        try:
            ...
            carb.log_warn(f"usd_mouse_interact: camera transform write failed: {exc}")

...
_SOURCE = "omni.mycompany.usd_mouse_interact.interaction_controller"
...
class InteractionController:
    ...
    def start(self) -> None:
        ...
        try:
            self._timeline_sub = self._timeline.get_timeline_event_stream().create_subscription_to_pop(
                self._on_timeline_event, name="usd_mouse_interact_timeline"
            )
        except TypeError:
            self._timeline_sub = (
        ...
        try:
            self._update_sub = (
                ...
                .create_subscription_to_pop(
                    self._on_update, name="usd_mouse_interact_update"
                )
            )
        except TypeError:
            self._update_sub = (
        ...
        try:
            ...
            self._stage_sub = stage_events.create_subscription_to_pop(
                self._on_stage_event, name="usd_mouse_interact_stage"
            )
        except TypeError:
            try:
                stage_events = omni.usd.get_context().get_stage_event_stream()
    ...
    def _on_update(self, event) -> None:
        ...
        keys = self._input_state.current()
        try:
            self._camera.apply(
                dx, dy, keys,

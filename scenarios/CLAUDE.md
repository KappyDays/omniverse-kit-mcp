...
```
`scenario_last_report` includes `failure_summary`, diagnostics/retry data plus `evidence_summary` for official verify, lidar, viewport framing, and capture evidence; start failed triage from Markdown `Failure Summary`, use Markdown for quick report reading, default JSON for exact fields, and `redact_local_paths=true` for public evidence. For live proof log capture, close with `extension_capture_logs(..., stop_after_capture=true)` and record `data.capture_stop_requested=true`, `data.capture_stop_completed=true`, `data.capture_stop_timed_out=false`, and `data.capture_running=false` before treating the capture hook as closed. `scenario_plan` exposes `total_steps`, `phase_counts`, `preflight_requirements`, `stage_mutation_summary`, `stage_mutation_steps`, `diagnostic_steps`, `evidence_steps`, `simulation_state_summary`, `simulation_state_steps`, `timeline_control_steps`, `retry_steps` with key args for retried evidence steps, `live_validation_checklist`, automatic cleanup, override `timeoutSeconds`, `idempotent`, `retries`, and `continueOnFailure`; check these before live runs, confirm `stage_mutation_summary.read_only=false` routes to scratch/test stage, and require `simulation_state_summary.play_state_missing_count=0` before robot or RTX sensor proof runs. `scenario_validate(..., dry_run=true)` returns the same plan fields plus `dry_run`, `steps`, and `compiled`. Authoring proof criteria: Robot+RTX live proof must assert `--expect-live-evidence-field read_lidar_point_cloud:status=passed`, `--expect-live-evidence-field-min read_lidar_point_cloud:num_points=1`, `frame_robot_and_sensors:bbox_empty=false`, `capture_visible_result:passed=true`; controlled lidar failure must assert `--expect-live-diagnostic-field read_lidar_point_cloud:diagnostics.reason=point_count_below_minimum`; official asset live proof must assert `official_asset_verify:verification_status=load_verified`, `official_asset_verify:kind=asset`, and `official_asset_verify:app_profile=isaac-sim`; official read-only catalog diagnostics must keep `stage_mutation_summary.read_only=true` and assert `search_known_miss:diagnostics.reason=query_no_match` plus `get_pallet_wrong_profile:diagnostics.reason=app_profile_not_covered`. After editing `src/omniverse_kit_mcp`, use `scripts/run_scenario_standalone.py --dry-run` to inspect the same plan shape before restarting a cached MCP host; for a normal standalone report that will become public evidence, add `--report-format markdown --redact-local-paths`.
Official asset scenario authoring gates: live verify proof must assert `--expect-live-status passed`, `official_asset_verify:verification_status=load_verified`, `official_asset_verify:kind=asset`, and `official_asset_verify:app_profile=isaac-sim`; read-only catalog diagnostics must keep `stage_mutation_summary.read_only=true` and assert `--expect-live-status passed`, `--expect-live-failure-step-error get_pallet_wrong_profile=OFFICIAL_ASSET_NOT_FOUND`, `--expect-live-diagnostic-next-actions-min 2`, `search_known_miss:diagnostics.reason=query_no_match`, and `get_pallet_wrong_profile:diagnostics.reason=app_profile_not_covered`.
Robot+RTX scenario authoring gate: success live proof must assert `--expect-live-status passed`.

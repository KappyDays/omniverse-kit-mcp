...
```
`scenario_last_report` includes `failure_summary`, diagnostics/retry data plus `evidence_summary` for official verify, lidar, viewport framing, and capture evidence; start failed triage from Markdown `Failure Summary`, use Markdown for quick report reading, default JSON for exact fields, and `redact_local_paths=true` for public evidence. `scenario_plan` exposes `total_steps`, `phase_counts`, `stage_mutation_steps`, `diagnostic_steps`, `evidence_steps`, `retry_steps` with key args for retried evidence steps, automatic cleanup, override `timeoutSeconds`, `idempotent`, `retries`, and `continueOnFailure`; check these before live runs and confirm `stage_mutation_steps` matches the scratch/test stage you intend to touch. `scenario_validate(..., dry_run=true)` returns the same plan fields plus `dry_run`, `steps`, and `compiled`. After editing `src/omniverse_kit_mcp`, use `scripts/run_scenario_standalone.py --dry-run` to inspect the same plan shape before restarting a cached MCP host; for a normal standalone report that will become public evidence, add `--report-format markdown --redact-local-paths`.

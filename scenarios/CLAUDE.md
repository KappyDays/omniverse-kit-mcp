...
```
`scenario_last_report` includes diagnostics/retry data plus `evidence_summary` for official verify, lidar, viewport framing, and capture evidence; use Markdown for triage, default JSON for exact fields, and `redact_local_paths=true` for public evidence. `scenario_plan` exposes `total_steps`, `phase_counts`, `evidence_steps`, `retry_steps`, automatic cleanup, override `timeoutSeconds`, `idempotent`, `retries`, and `continueOnFailure`; check these before live runs. `scenario_validate(..., dry_run=true)` uses the same executable count model.

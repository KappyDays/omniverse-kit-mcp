...
```
`scenario_last_report` includes diagnostics/retry data; use Markdown for triage, default JSON for exact fields, and `redact_local_paths=true` for public evidence. `scenario_plan` exposes override `timeoutSeconds`, `idempotent`, `retries`, and `continueOnFailure`; check these before live runs.
